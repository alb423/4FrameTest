...
-(IBAction)play1Action:(id)sender {
    ...
    [self.view insertSubview:myGLView atIndex:0];
#else
    // do nothing
#endif
...
-(IBAction)play2Action:(id)sender {
    ...
    #if PLAY_MEHTOD == PLAY_MEMORY_FILE
    video1 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC1]];
    video2 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC2]];
    [video1 seekTime:0.0];
    [video2 seekTime:0.0];
    #else
    // do nothing
...
-(IBAction)play4Action:(id)sender {
    ...
    self->video1 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC1]];
    self->video2 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC2]];
    self->video3 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC4]];
    ...
    video1 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC1]];
    video2 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC2]];
    video3 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC3]];
    video4 = [[VideoFrameExtractor alloc] initWithVideoMemory:[Utilities bundlePath:VIDEO_SRC4]];
    [video1 seekTime:0.0];
    [video2 seekTime:0.0];
    [video3 seekTime:0.0];
    [video4 seekTime:0.0];
...
-(void)displayNextFrame_Optimized:(NSTimer *)timer {
    ...
    {
        // update the estimate time
        [DecodeLabel setText:[NSString stringWithFormat:@"%f", (vDecodeTime/self.FPS/1000000)]];
        [ShowImageLabel setText:[NSString stringWithFormat:@"%f", (vShowImageTime/self.FPS/1000000)]];
